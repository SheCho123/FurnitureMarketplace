class AddArticle extends Screen {
    ...
    SwitchTo(){
        ...
        var sviSuTu = [nameInputId, priceInputId, imageInputId, typeInputId];
    
        magicnoDugme.addEventListener('click', ()=>{
            for(let i=0;i<sviSuTu.length;i++){
                if(sviSuTu[i].value == "" || sviSuTu[i].value == null)
                sviSuTu[i].classList.add('redBorder')
            }
